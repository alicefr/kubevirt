...
var _ = SIGDescribe("SCSI persistent reservation", func() {
	...
	BeforeEach(func() {
		checks.SkipTestIfNoFeatureGate(virtconfig.PersistentReservationHelper)
		var err error
	...
	Context("Use LUN disk with presistent reservation", func() {
		FIt("Should successfully start a VM with persistent reservation", func() {
			...
			Expect(console.SafeExpectBatch(vmi, []expect.Batcher{
				...
				&expect.BExp{R: `  LIO-ORG   disk1             4.0
  Peripheral device type: disk
  PR generation=0x0, there are NO registered reservation keys`},
				&expect.BSnd{S: "sg_persist -o -G  --param-sark=12345678 /dev/sda"},
				&expect.BExp{R: `LIO-ORG   disk0             4.0
  Peripheral device type: disk`},
				&expect.BSnd{S: "sg_persist  -o -R --prout-type 5  --param-rk=12345678 /dev/sda"},
				&expect.BExp{R: `  LIO-ORG   disk0             4.0
  Peripheral device type: disk`},
				&expect.BSnd{S: "sg_persist -i -r /dev/sda"},
				&expect.BExp{R: `  LIO-ORG   disk0             4.0
  Peripheral device type: disk
  PR generation=0x1, Reservation follows:
    Key=0x12345678
    scope: LU_SCOPE,  type: Write Exclusive, registrants only`},
				&expect.BSnd{S: "sg_persist -o -L --prout-type 5 --param-rk=12345678 /dev/sda"},
				&expect.BExp{R: `  LIO-ORG   disk0             4.0 
  Peripheral device type: disk`},
			}, 20)).To(Succeed())
