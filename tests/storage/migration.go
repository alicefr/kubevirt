...
var _ = SIGDescribe("Storage migration", func() {
	...
	waitForVolumeMigrationForVMIToComplete := func(smName, ns string, seconds int) {
		...
		}, seconds, 1*time.Second).Should(gomega.BeTrue(), fmt.Sprintf("The storage migration %s should be completed", smName))
	...
	Describe("Storage migration", func() {
		...
		DescribeTable("storage migration with a single VM", func(noAddDisks int, mode modePVC) {
			sc := setSC(mode)
			// Additional disks
			...
			for src, _ := range addDisks {
				tests.AddPVCDisk(vmi, src, v1.DiskBusVirtio, src)
			}
					...
					MigratedVolume: []virtstoragev1alpha1.MigratedVolume{
						{
							...
							SourcePvcReclaimPolicy: virtstoragev1alpha1.SourcePvcReclaimPolicyDelete,
						},
					},
				},
			}
