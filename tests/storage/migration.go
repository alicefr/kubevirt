	...
	Describe("Update volumes with the migration updateVolumesStrategy", func() {
		...
		checkMigratedVolumesOnVM := func(vm *virtv1.VirtualMachine, volName, src, dst string, succeded bool) {
			...
			Eventually(func() bool {
				vm, err := virtClient.VirtualMachine(vm.Namespace).Get(context.Background(), vm.Name, metav1.GetOptions{})
				Expect(err).ToNot(HaveOccurred())
				return controller.NewVolumeMigrationConditionManager().HasConditionWithStatus(
					vm.Status.VolumeMigration, virtv1.VolumeMigrationConditionCompleted, cond)
		...
		FIt("should migrate the source volume from a source DV to a destination DV", func() {
			volName := "disk0"
			dv := createDV()
		...
		FIt("should keep the migrated volumes information in the VM status if the VM is shutdown", func() {
			volName := "volume"
			dv := createDV()
			vm := createVMWithDV(dv, volName)
			// Create dest PVC
