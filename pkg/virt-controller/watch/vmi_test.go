...
var _ = Describe("VirtualMachineInstance watcher", func() {
	...
	Context("On valid VirtualMachineInstance given", func() {
		...
		It("should add outdated label if pod's image is outdated and VMI is in running state", func() {
			...
			podPatch, err := patch.PatchTestReplaceLabels(map[string]string{"kubevirt.io": "virt-launcher", "kubevirt.io/created-by": "1234"},
		...
		It("should add a ready condition if it is present on the pod and the VMI is in running state", func() {
			...
			patch, err := patch.PatchTestReplaceConditions(nil, []virtv1.VirtualMachineInstanceCondition{{
		...
		Context("should update pod labels", func() {
			...
			DescribeTable("when VMI dynamic label set changes", func(td *testData, expectOldLabels, expectNewLabels map[string]string) {
				...
				if expectOldLabels != nil && expectNewLabels != nil {
					expectedPatch, err = patch.PatchTestReplaceLabels(expectOldLabels, expectNewLabels)
				}
				Expect(err).ToNot(HaveOccurred())
	...
	Context("hotplug volume", func() {
		...
		It("Should get default filesystem overhead if there are multiple CDI instances", func() {
			...
			fsOverhead, err := storagetypes.GetFilesystemOverheadInformers(controller.cdiInformer,
				controller.cdiConfigInformer,
