...
var _ = Describe("Volume Migration", func() {
	Context("ValidateVolumes", func() {
		DescribeTable("should validate the migrated volumes", func(vmi *virtv1.VirtualMachineInstance, vm *virtv1.VirtualMachine, expectValid bool, expectErrorMsg string) {
			valid, errMsg := ValidateVolumes(vmi, vm)
			Expect(valid).To(Equal(expectValid))
			Expect(errMsg).To(Equal(expectErrorMsg))
		},
			Entry("with empty VMI", nil, &virtv1.VirtualMachine{}, false, "cannot validate the migrated volumes for an empty VMI"),
			Entry("with empty VM", &virtv1.VirtualMachineInstance{}, nil, false, "cannot validate the migrated volumes for an empty VM"),
			...
			)), false, "invalid volumes to update with migration: luns: [disk1]"),
			Entry("with an invalid lun volume", libvmi.New(
				libvmi.WithPersistentVolumeClaim("disk0", "vol0"), libvmi.WithPersistentVolumeClaimLun("disk1", "vol1", false),
			), libvmi.NewVirtualMachine(libvmi.New(
				libvmi.WithPersistentVolumeClaim("disk0", "vol2"), libvmi.WithPersistentVolumeClaimLun("disk1", "vol4", false),
			...
			)), false, "invalid volumes to update with migration: shareable: [disk1]"),
			...
			)), false, "invalid volumes to update with migration: filesystems: [disk1]"),
			Entry("with an invalid filesystem volume", libvmi.New(
				libvmi.WithPersistentVolumeClaim("disk0", "vol0"), withFilesystemVolume("disk1", "vol1"),
			), libvmi.NewVirtualMachine(libvmi.New(
				libvmi.WithPersistentVolumeClaim("disk0", "vol2"), withFilesystemVolume("disk1", "vol4"),
			...
			)), false, "invalid volumes to update with migration: hotplugged: [disk1]"),
			...
			)), false, "invalid volumes to update with migration: DVs: [disk1]"),
			Entry("with an invalid datavolume template", libvmi.New(
				libvmi.WithPersistentVolumeClaim("disk0", "vol0"), libvmi.WithDataVolume("disk1", "vol1"),
			), libvmi.NewVirtualMachine(libvmi.New(
				libvmi.WithPersistentVolumeClaim("disk0", "vol2"), libvmi.WithPersistentVolumeClaim("disk1", "vol3"),
			), withDataVolumeTemplate("disk1")), false, "invalid volumes to update with migration: DV templates: [disk1]"),
		)
	})

	Context("VolumeMigrationAbortion", func() {
		var (
		...
		shouldPatchVMI := func(vmi *virtv1.VirtualMachineInstance) {
			...
			vmiInterface.EXPECT().Patch(context.Background(), vmi.Name, types.JSONPatchType, gomock.Any(), metav1.PatchOptions{}).
				Do(func(ctx context.Context, name, patchType, patch, opts interface{}, subs ...interface{}) {
					...
					Expect(condManager.HasCondition(newVMI, virtv1.VirtualMachineInstanceChangeAbortion)).To(BeTrue())
					Expect(newVMI.Status.MigratedVolumes).To(HaveLen(0))
				})
		}
		DescribeTable("should evaluate the volume migration abortion", func(vmiVols, vmVols []string, migVols []migVolumes, expectRes bool, expectErr error, expectAbort bool) {
			...
			res, err := VolumeMigrationAbortion(virtClient, vmi, vm)
			if expectErr != nil {
			...
			Entry("without any updates", []string{"dst0"}, []string{"dst0"}, []migVolumes{{diskName(0), "src0", "dst0"}}, false, nil, false),
			Entry("with the migrated volumes reversion to the source volumes", []string{"dst0"}, []string{"src0"},
				[]migVolumes{{diskName(0), "src0", "dst0"}}, true, nil, true),
			Entry("with invalid update", []string{"dst0"}, []string{"other"}, []migVolumes{{diskName(0), "src0", "dst0"}}, true,
				fmt.Errorf(invalidUpdateErrMsg), false),
			Entry("with invalid partial update", []string{"dst0", "dst1", "dst2"}, []string{"src0", "dst1", "dst2"}, []migVolumes{
				{diskName(0), "src0", "dst0"}, {diskName(1), "src1", "dst1"}, {diskName(2), "src2", "dst2"}},
				true, fmt.Errorf(invalidUpdateErrMsg), false),
		)
	})
	...
	Context("IsVolumeMigrating", func() {
		DescribeTable("should detect the volume update condition", func(cond *virtv1.VirtualMachineInstanceCondition, expectRes bool) {
			...
			Expect(IsVolumeMigrating(vmi)).To(Equal(expectRes))
		},
	...
	Context("PatchVMIStatusWithMigratedVolumes", func() {
			...
			vmiInterface.EXPECT().Patch(context.Background(), vmi.Name, types.JSONPatchType, gomock.Any(), metav1.PatchOptions{}).
				Do(func(ctx context.Context, name, patchType, patch, opts interface{}, subs ...interface{}) {
					...
					for _, migVol := range vmi.Status.MigratedVolumes {
						...
						Expect(migVol.SourcePVCInfo.VolumeMode).To(Equal(k8sv1.PersistentVolumeFilesystem))
						Expect(migVol.DestinationPVCInfo.VolumeMode).To(Equal(k8sv1.PersistentVolumeFilesystem))
					}
				})
		}
		DescribeTable("should update the migrated volumes in the vmi", func(vmiVols, vmVols []string, expectedMigVols map[string]migVolumes) {
			...
			Expect(PatchVMIStatusWithMigratedVolumes(virtClient, pvcStore, vmi, vm)).ToNot(HaveOccurred())
		},
			Entry("with an update of a volume", []string{"src0"}, []string{"dst0"},
				map[string]migVolumes{diskName(0): migVolumes{src: "src0", dst: "dst0"}}),
			Entry("with an update of multiple volumes", []string{"src0", "src1"}, []string{"dst0", "dst1"},
				map[string]migVolumes{diskName(0): migVolumes{src: "src0", dst: "dst0"},
					diskName(1): migVolumes{src: "src1", dst: "dst1"}}),
...
func addVMIOptionsForVolumes(vols []string) []libvmi.Option {
	...
	for i, v := range vols {
		ops = append(ops, libvmi.WithPersistentVolumeClaim(diskName(i), v))
	}
	return ops
}
