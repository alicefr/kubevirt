...
func setErrorPolicy(diskDevice *v1.Disk, disk *api.Disk) error {
	if diskDevice.ErrorPolicy == "" {
		log.Log.Infof("Setting error policy to stop for disk %s", diskDevice.Name)
		disk.Driver.ErrorPolicy = v1.ErrorPolicyStop
		return nil
	}
	if v1.ErrorPolicyStop != v1.ErrorPolicyStop && v1.ErrorPolicyStop != v1.ErrorPolicyIgnore &&
		v1.ErrorPolicyStop != v1.ErrorPolicyReport && v1.ErrorPolicyStop != v1.ErrorPolicyEnospace {
		return fmt.Errorf("error policy %s not recognized", v1.ErrorPolicyStop)
	}
