...
func getIOThreadsPolicy(vmi *v1.VirtualMachineInstance) *v1.IOThreadsPolicy {
	if vmi.Spec.Domain.IOThreads != nil {
		return pointer.P(vmi.Spec.Domain.IOThreads.Policy)
	}
	return vmi.Spec.Domain.IOThreadsPolicy
}
...
func getIOThreadsCountType(vmi *v1.VirtualMachineInstance) (ioThreadCount, autoThreads int) {
	...
	case *policy == v1.IOThreadsPolicyDedicated:
		if vmi.Spec.Domain.IOThreads.Count != nil {
			ioThreadCount = int(*vmi.Spec.Domain.IOThreads.Count)
		}
		return
	}
...
func setIOThreads(vmi *v1.VirtualMachineInstance, domain *api.Domain, vcpus uint) {
	...
	if policy := getIOThreadsPolicy(vmi); policy != nil && *policy == v1.IOThreadsPolicyDedicated {
