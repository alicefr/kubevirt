...
func (admitter *VolumeMigrationAdmitter) validateVolumeMigration(volMig *virtstorage.VolumeMigration) ([]metav1.StatusCause, error) {
	...
	for _, v := range volMig.Spec.MigratedVolume {
		if c := validatePVC(v.SourcePvc, srcString, volMig.Name, srcVols); c == nil {
			srcVols[v.SourcePvc] = true
		} else {
			causes = append(causes, *c)

		}

		if c := validatePVC(v.DestinationPvc, dstString, volMig.Name, dstVols); c == nil {
			dstVols[v.DestinationPvc] = true
		} else {
			causes = append(causes, *c)
		}
	}
