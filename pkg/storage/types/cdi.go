...
func GetFilesystemOverheadInformers(cdiStore cache.Store, cdiConfigStore cache.Store, pvc *k8sv1.PersistentVolumeClaim) (cdiv1.Percent, error) {
	// To avoid conflicts, we only allow having one CDI instance
	if cdiInstances := len(cdiStore.List()); cdiInstances != 1 {
		if cdiInstances > 1 {
	...
	cdiConfigInterface, cdiConfigExists, err := cdiConfigStore.GetByKey(ConfigName)
