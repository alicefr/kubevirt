...
var _ = Describe("VolumeMigrationUpdater", func() {
	...
	Context("Update volumes on the VMI", func() {
		...
		It("should not update volumes if volume migration hasn't succeeded", func() {
			...
			migVols := []virtstoragev1alpha1.MigratedVolume{{SourcePvc: "src", DestinationPvc: "dest"}}
	...
	Context("Update volumes on the VM", func() {
		Context("Update volumes after successful migration on a VMI", func() {
			...
			DescribeTable("UpdateVMWithMigrationVolumes",
				...
				Entry("successful update simple VM", []string{"src1"}, []virtstoragev1alpha1.MigratedVolume{{SourcePvc: "src1", DestinationPvc: "dest1"}}, nil),
				Entry("successful update VM with multiple PVCs", []string{"src1", "src2", "src3"}, []virtstoragev1alpha1.MigratedVolume{{SourcePvc: "src1", DestinationPvc: "dest1"}}, nil),
