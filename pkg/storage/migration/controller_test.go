...
var _ = Describe("Volume Migration", func() {
	...
	Context("VolumeMigration lifecycle", func() {
		const (
			...
			volMigName    = "vol-mig"
			vmiName       = "testVmi"
			defaultPolicy = virtstoragev1alpha1.SourcePvcReclaimPolicyDelete
		)
		...
		createVolMig := func(name string,
			policy virtstoragev1alpha1.SourcePvcReclaimPolicy) *virtstoragev1alpha1.VolumeMigration {
			return &virtstoragev1alpha1.VolumeMigration{
				...
				Spec: virtstoragev1alpha1.VolumeMigrationSpec{
					MigratedVolume: []virtstoragev1alpha1.MigratedVolume{
						{
							SourcePvc:              srcVol,
							DestinationPvc:         dstVol,
							SourcePvcReclaimPolicy: policy,
						},
					},
				},
			}
		...
		createMig := func(name string) *virtv1.VirtualMachineInstanceMigration {
			return &virtv1.VirtualMachineInstanceMigration{
				ObjectMeta: metav1.ObjectMeta{
					Name:      name,
					Namespace: testNs,
					Labels:    map[string]string{labelVolumeMigration: volMigName},
				},
			}
		}

		initVolMigCreation := func(vmOwner bool, policy virtstoragev1alpha1.SourcePvcReclaimPolicy) *virtstoragev1alpha1.VolumeMigration {
		...
		DescribeTable("Source PVC reclaim policy", func(policy string) {
			p := virtstoragev1alpha1.SourcePvcReclaimPolicy(policy)
			initMigration(createMigState(true, false), false, p)
			migrationInterface.EXPECT().Delete(gomock.Any(), gomock.Any()).Times(1).Return(nil)
			if p == virtstoragev1alpha1.SourcePvcReclaimPolicyDelete {
				expectPVCDeletion()
			}

			controller.Execute()
		},
			Entry("with delete policy", string(virtstoragev1alpha1.SourcePvcReclaimPolicyDelete)),
			Entry("with retain policy", string(virtstoragev1alpha1.SourcePvcReclaimPolicyRetain)),
	...
	Context("classifyVolumesPerVMI", func() {
		...
		migVols := []virtstoragev1alpha1.MigratedVolume{
			{SourcePvc: srcVolName, DestinationPvc: dstVolName}}
		migVol2 := virtstoragev1alpha1.MigratedVolume{
			SourcePvc: "src2", DestinationPvc: "dst2",
		}
	...
	Context("validateMigrateVolumes", func() {
		...
		migVols := []virtstoragev1alpha1.MigratedVolume{
			{SourcePvc: srcVolName1, DestinationPvc: dstVolName1},
			{SourcePvc: srcVolName2, DestinationPvc: dstVolName2},
		}
		...
		DescribeTable("evaluate rejected volumes for the volume migration",
			func(vmi *virtv1.VirtualMachineInstance, reason string) {
				...
				state := virtstoragev1alpha1.MigratedVolumeStateRejected
				r := &reason
				if reason == "" {
					state = virtstoragev1alpha1.MigratedVolumeStateValid
					r = nil
				}
				...
				Expect(rejMig.Status.VolumeMigrationStates[1].State).To(
					Equal(virtstoragev1alpha1.MigratedVolumeStateValid))
				Expect(rejMig.Status.VolumeMigrationStates[0].Reason).To(Equal(r))
				Expect(rejMig.Status.VolumeMigrationStates[0].State).To(
					Equal(state))
			},
