...
type KubeInformerFactory interface {
	...
	// StorageMigration
	StorageMigration() cache.SharedIndexInformer
}
...
func (f *kubeInformerFactory) StorageMigration() cache.SharedIndexInformer {
	return f.getInformer("storageMigrationInformer", func() cache.SharedIndexInformer {
		lw := cache.NewListWatchFromClient(f.clientSet.GeneratedKubeVirtClient().StorageV1alpha1().RESTClient(), "storagemigrations", k8sv1.NamespaceAll, fields.Everything())
		return cache.NewSharedIndexInformer(lw, &virtstoragev1alpha1.StorageMigration{}, f.defaultResync, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
	})
}
