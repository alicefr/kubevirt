...
const (
	mountBaseDir = "/var/run/kubevirt-ephemeral-disks/container-disk-data"
...
func GetContainerDiksPath(volume *v1.Volume) (string, error) {
	...
	path := volume.VolumeSource.ContainerDisk.Path
	if path == "" {
		path = DiskSourceFallbackPath
	}
	fullPath := filepath.Join("/proc", strconv.Itoa(pid), "/root", path)
	files, err := os.ReadDir(fullPath)
	if err != nil {
		return "", err
	}
	if len(files) == 0 {
		return "", fmt.Errorf("no file found in folder %s, no disk present", path)
	} else if len(files) > 1 {
...
func CreateEphemeralImages(
	...
	for i, volume := range vmi.Spec.Volumes {
		...
		info, err := GetImageInfo(backingFile)
		if err != nil {
			return err
		}
