...
func NewMounter(isoDetector isolation.PodIsolationDetector, clusterConfig *virtconfig.ClusterConfig, mountRecorder mountutils.MountRecorder) Mounter {
	return &mounter{
...
func (m *mounter) unmountKernelArtifacts(vmi *v1.VirtualMachineInstance) error {
	...
	for _, entry := range record {
		...
		//
		//		removeSliceElement := func(s []mountutils.ContainerDisksMountTargetEntry, idxToRemove int) []mountutils.ContainerDisksMountTargetEntry {
		//			// removes slice element efficiently
		//			s[idxToRemove] = s[len(s)-1]
		//			return s[:len(s)-1]
		//		}
		//
		//		record = removeSliceElement(record, idx)
		return nil
	}
